...
public:
     ...
     *      -1 失败
     */
    static int trim_space(char *inbuf);
     ...
     *          0 成功, -1 失败
     */
    static int query_parse_key_value(const char *query, const char *key, char *value, int *value_len_p);

    //通过文件名file_name， 得到文件后缀字符串, 保存在suffix 如果非法文件后缀,返回"null"
    static int get_file_suffix(const char *file_name, char *suffix);
    ...
    *          成功返回0，失败-1
    */
    static int verify_token(char *user, char *token);
