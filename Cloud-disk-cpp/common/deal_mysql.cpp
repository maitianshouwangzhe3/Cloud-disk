 ...
 */
/* -------------------------------------------*/
void deal_mysql::print_error(MYSQL *conn, const char *title){
 ...
 */
/* -------------------------------------------*/
MYSQL* deal_mysql::msql_conn(char *user_name, char* passwd, char *db_name){
 ...
 */
/* -------------------------------------------*/
void deal_mysql::process_result_test(MYSQL *conn, MYSQL_RES *res_set){
    MYSQL_ROW row;
    int i;
...
/* -------------------------------------------*/
/**
 * @brief  处理数据库查询结果，结果集保存在buf，只处理一条记录，
 * 一个字段, 如果buf为NULL，无需保存结果集，只做判断有没有此记录
 * @param sql_cmd            sql语句
 * @param buf                返回结果
 *
