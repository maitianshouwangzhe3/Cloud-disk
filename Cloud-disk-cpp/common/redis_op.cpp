 ...
 */
/* -------------------------------------------*/
int redis_op::rop_selectdatabase(redisContext *conn, unsigned int db_no)
{
	int retn = 0;
 ...
 */
/* -------------------------------------------*/
int redis_op::rop_flush_database(redisContext *conn)
{
	int retn = 0;	
	redisReply *reply = NULL;
 ...
 */
/* -------------------------------------------*/
int redis_op::rop_is_key_exist(redisContext *conn, char* key)
{
	int retn = 0;	
 ...
 */
/* -------------------------------------------*/
int redis_op::rop_del_key(redisContext *conn, char *key)
{
	int retn = 0;
	redisReply *reply = NULL;
 ...
 */
/* -------------------------------------------*/
int redis_op::rop_set_key_lifecycle(redisContext *conn, char *key, time_t delete_time)
{
	int retn = 0;
	redisReply *reply = NULL;		
 ...
 */
/* -------------------------------------------*/
void redis_op::rop_show_keys(redisContext *conn, char* pattern)
{
	int i = 0;
	redisReply *reply = NULL;
 ...
 */
/* -------------------------------------------*/
int redis_op::rop_redis_append(redisContext *conn, RCOMMANDS cmds, int cmd_num)
{
	int retn = 0;
	int i = 0;
	redisReply *reply = NULL;
 ...
 */
/* -------------------------------------------*/
int redis_op::rop_redis_command(redisContext *conn, char *cmd)
{
	int retn = 0;
 ...
 */
/* -------------------------------------------*/
void redis_op::rop_test_reply_type(redisReply *reply)
{
	switch (reply->type) {
 ...
 */
/* -------------------------------------------*/
redisContext* redis_op::rop_connectdb_nopwd(char *ip_str, char* port_str)
{
 ...
 */
/* -------------------------------------------*/
redisContext* redis_op::rop_connectdb(char *ip_str, char* port_str, char *pwd)
{
	redisContext *conn = NULL;
	uint16_t port = atoi(port_str);
 ...
 */
/* -------------------------------------------*/
redisContext* redis_op::rop_connectdb_unix(char *sock_path, char *pwd)
{
	redisContext *conn = NULL;
    char auth_cmd[REDIS_COMMAND_SIZE];
 ...
 */
/* -------------------------------------------*/
redisContext* redis_op::rop_connectdb_timeout(char* ip_str, char *port_str, struct timeval *timeout)
{
	redisContext *conn = NULL;
	uint16_t port = atoi(port_str);
 ...
 */
/* -------------------------------------------*/
void redis_op::rop_disconnect(redisContext* conn)
{
	if (conn == NULL) {
		return ;
	}
 ...
 */
/* -------------------------------------------*/
int redis_op::rop_hash_set_append(redisContext *conn, char *key, RFIELDS fields, RVALUES values, int val_num)
{
    int retn = 0;
    int i = 0;
    redisReply *reply = NULL;
 ...
 */
/* -------------------------------------------*/
int redis_op::rop_hash_set(redisContext *conn, char *key, char *field, char *value)
{
    int retn = 0;
    redisReply *reply = NULL;
 ...
 */
/* -------------------------------------------*/
int redis_op::rop_hash_get(redisContext *conn, char *key, char *field, char *value)
{
    int retn = 0;
    int len = 0;
 ...
 */
/* -------------------------------------------*/
int redis_op::rop_hash_del(redisContext *conn, char *key, char *field)
{
    int retn = 0;
    redisReply *reply = NULL;
 ...
 */
/* -------------------------------------------*/
int redis_op::rop_create_or_replace_hash_table(redisContext* conn,
									 char* key, 
 ...
 */
/* -------------------------------------------*/
int redis_op::rop_hincrement_one_field(redisContext *conn, char *key, char *field, unsigned int num)
{
	int retn = 0;
 ...
 */
/* -------------------------------------------*/
int redis_op::rop_list_push_append(redisContext *conn, char *key, RVALUES values, int val_num)
{
	int retn = 0;
	int i = 0;
	redisReply *reply = NULL;
 ...
 */
/* -------------------------------------------*/
int redis_op::rop_list_push(redisContext *conn, char *key, char *value)
{
	int retn = 0;
	redisReply *reply = NULL;
 ...
 */
/* -------------------------------------------*/
int redis_op::rop_get_list_cnt(redisContext *conn, char *key)
{
	int cnt = 0;
 ...
 */
/* -------------------------------------------*/
int redis_op::rop_trim_list(redisContext *conn, char *key, int begin, int end)
{
	int retn = 0;
	redisReply *reply = NULL;
 ...
 */
/* -------------------------------------------*/
int redis_op::rop_range_list(redisContext *conn, char *key, int from_pos, int end_pos, RVALUES values, int *get_num)
{
    int retn = 0;
    int i = 0;
    redisReply *reply = NULL;
...
int redis_op::rop_set_string(redisContext *conn, char *key, char *value)
{
    int retn = 0;
	redisReply *reply = NULL;
...
int redis_op::rop_setex_string(redisContext *conn, char *key, unsigned int seconds, char *value)
{
    int retn = 0;
    redisReply *reply = NULL;
...
int redis_op::rop_get_string(redisContext *conn, char *key, char *value)
{
    int retn = 0;
	redisReply *reply = NULL;
 ...
 */
/* -------------------------------------------*/
int redis_op::rop_zset_add(redisContext *conn, char* key, long score, char* member)
{
    int retn = 0;
    redisReply *reply = NULL;
 ...
 */
/* -------------------------------------------*/
int redis_op::rop_zset_zrem(redisContext *conn, char* key, char* member)
{
    int retn = 0;
    redisReply *reply = NULL;
 ...
 */
/* -------------------------------------------*/
int redis_op::rop_zset_del_all(redisContext *conn, char* key)
{
    int retn = 0;
    redisReply *reply = NULL;
 ...
 */
/* -------------------------------------------*/
int redis_op::rop_zset_zrevrange(redisContext *conn, char *key, int from_pos, int end_pos, RVALUES values, int *get_num)
{
    int retn = 0;
    int i = 0;
    redisReply *reply = NULL;
 ...
 */
/* -------------------------------------------*/
int redis_op::rop_zset_increment(redisContext *conn, char* key, char* member)
{
    int retn = 0;
 ...
 */
/* -------------------------------------------*/
int redis_op::rop_zset_zcard(redisContext *conn, char *key)
{
    int cnt = 0;
 ...
 */
/* -------------------------------------------*/
int redis_op::rop_zset_get_score(redisContext *conn, char *key, char *member)
{
    int score = 0;
 ...
 */
/* -------------------------------------------*/
int redis_op::rop_zset_exit(redisContext *conn, char *key, char *member)
{
    int retn = 0;
    redisReply *reply = NULL;
 ...
 */
/* -------------------------------------------*/
int redis_op::rop_zset_increment_append(redisContext *conn, char *key, RVALUES values, int val_num)
{
    int retn = 0;
    int i = 0;
    redisReply *reply = NULL;
