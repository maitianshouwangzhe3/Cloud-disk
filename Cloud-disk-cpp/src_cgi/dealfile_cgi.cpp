...
void dealfile::aftermath(int ret){
     /*
    下载文件pv字段处理
        成功：{"code":"016"}
        失败：{"code":"017"}
...
void dealfile::aftermath(char* out, int ret, redisContext* redis_conn, redis_op& rp){
    /*
    删除文件：
        成功：{"code":"013"}
        失败：{"code":"014"}
    */
    ...
    if(ret == 0)
    {
        out = util_cgi::return_status("013");
    }
    else
    {
        out = util_cgi::return_status("014");
    }
...
void dealfile::aftermath(int ret, redisContext* redis_conn, redis_op& rp){
     /*
   分享文件：
        成功：{"code":"010"}
        失败：{"code":"011"}
    ...
    */
    ...
    if(ret == 0)
    {
        out = util_cgi::return_status("010");
    }
    else if(ret == -1)
    {
        out = util_cgi::return_status("011");
    }
    else if(ret == -2)
    {
        out = util_cgi::return_status("012");
    }
...
int dealfile::del_file(){
    ...
    */
    ...
    if (redis_conn == NULL)
    {
        ...
        ret = -1;
        //善后
        aftermath(out, ret, redis_conn, rp);
        return ret;
    }
     //connect the database
    conn = deal_mysql::msql_conn(mysql_user, mysql_pwd, mysql_db);
    ...
    if(share == 1)
    {
        ...
        {
            //有序集合删除指定成员
            rp.rop_zset_zrem(redis_conn, FILE_PUBLIC_ZSET, fileid);

            //从hash移除相应记录
            rp.rop_hash_del(redis_conn, FILE_NAME_HASH, fileid);
        }
...
int dealfile::remove_file_from_storage(char *fileid){
    int ret = 0;
    ...
    */
    ...
    {//===2、如果集合没有此元素，可能因为redis中没有记录，再从mysql中查询，如果mysql也没有，说明真没有(mysql操作)
     //===3、如果mysql有记录，而redis没有记录，说明redis没有保存此文件，redis保存此文件信息后，再中断操作(redis操作)
        ...
        {
            //redis保存此文件信息
            rp.rop_zset_add(redis_conn, FILE_PUBLIC_ZSET, 0, fileid);
