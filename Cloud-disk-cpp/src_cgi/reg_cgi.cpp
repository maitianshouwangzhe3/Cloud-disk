    ...
    while(FCGX_Accept_r(&request) == 0){
        ...
        if(len <= 0){//没有用户登录信息
           ...
           if (ret == 0) //登陆成功
            {
                //返回前端注册情况， 002代表成功
                out = util_cgi::return_status("002");//util_cgi.h
            }
            else if(ret == -1)
            {
                //返回前端注册情况， 004代表失败
                out = util_cgi::return_status("004");//util_cgi.h
            }
            else if(ret == -2)
            {
               out = util_cgi::return_status("003");//util_cgi.h
            }
...
int reg::user_register(){
    ...
    //char mysql_db[256] = {0};
    ret = cfg::get_mysql_info(mysql_user, mysql_pwd,  mysql_db);
    ...
    char tel[128];
    char email[128];
    ret = get_reg_info(buf, user, nick_name, pwd, tel, email);
    if(ret != 0)
    {
        return ret;
    }
    ...
    ret2 = deal_mysql::process_result_one(conn, sql_cmd, NULL); //指向sql查询语句
    if(ret2 == 2) //如果存在
    {
...
//解析用户注册信息的json包
int reg::get_reg_info(char *reg_buf, char *user, char *nick_name, char *pwd, char *tel, char *email)
{
    int ret = 0;
