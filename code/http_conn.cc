...
void CHttpConn::OnRead(Buffer *buf) // CHttpConn业务层面的OnRead
{
  ...
  http_parser_.ParseHttpContent(
      in_buf, buf_len); // 1. 从socket接口读取数据；2.然后把数据放到buffer
                        // in_buf; 3.http解析
  if (http_parser_.IsReadAll()) {
    string url = http_parser_.GetUrlString();
    string content = http_parser_.GetBodyContentString();
    LOG_INFO("url: {}, content: {}, content_type {}", url, content,
             http_parser_.GetContentType());
    ...
    } /*else if (strncmp(url.c_str(), "/share", 6) == 0) { //  GET sharefile
      _HandleGetShare(url, content);
    } */
    else {
...
int CHttpConn::_HandleGetShare(string &url, string &post_data) {
  char *szContent = new char[HTTP_RESPONSE_JSON_MAX];
  std::string strUrlMd5 = "";
  char urlmd5[64] = {0};
  int urlmd5_len = 0;
  std::string strJson = "";
  uint32_t ulen = 0;

  QueryParseKeyValue(url.c_str(), "urlmd5", urlmd5, &urlmd5_len);
  if (urlmd5_len == 0) {
    LOG_ERROR("urlmd5 is null");
    goto error;
  }
  strUrlMd5 = std::string(urlmd5, urlmd5_len);

  ApiSharep(strUrlMd5, strJson);
  ulen = strJson.length();
  snprintf(szContent, HTTP_RESPONSE_JSON_MAX, HTTP_RESPONSE_JSON, ulen,
           strJson.c_str());
error:
